...
export function ChunkEditor({
  ...
  const editorCard = (
    ...
    >
      ...
      {/* Field: TTS 源 */}
      <FieldRow label="TTS 源" fieldKey="text_normalized" hint="改 → 重新合成" hintWarn>
        {editingField === "tts" ? (
          <textarea
            ref={ttsRef}
            value={ttsValue}
            onChange={(e) => { setTtsValue(e.target.value); autoResize(e.currentTarget); }}
            onBlur={handleBlur}
          ...
          <div
            ...
            className="text-[11px] text-neutral-800 dark:text-neutral-200 leading-relaxed px-1 py-0.5 rounded-[3px] cursor-text border border-transparent hover:bg-neutral-100 dark:hover:bg-neutral-800 hover:border-neutral-200 dark:hover:border-neutral-700 min-h-[18px] whitespace-pre-wrap break-words"
          >
            {ttsValue}
          </div>
        )}
      ...
      {/* Field: 字幕 */}
      <FieldRow label="字幕" fieldKey="subtitle_text" hint="改 → 重生字幕">
        {editingField === "sub" ? (
          <textarea
            ref={subRef}
            value={subValue}
            placeholder={subPlaceholder}
            onChange={(e) => { setSubValue(e.target.value); autoResize(e.currentTarget); }}
            onBlur={handleBlur}
            onKeyDown={handleKeyDown}
            className="w-full text-[11px] text-neutral-800 dark:text-neutral-200 leading-relaxed px-1 py-0.5 border border-neutral-800 dark:border-neutral-400 rounded-[3px] bg-white dark:bg-neutral-800 font-inherit resize-none outline-none"
          />
          ...
          <div
            ...
            className={`text-[11px] leading-relaxed px-1 py-0.5 rounded-[3px] cursor-text border border-transparent hover:bg-neutral-100 dark:hover:bg-neutral-800 hover:border-neutral-200 dark:hover:border-neutral-700 min-h-[18px] whitespace-pre-wrap break-words ${
              subDisplayEmpty ? "text-neutral-400 dark:text-neutral-500 italic" : "text-neutral-800 dark:text-neutral-200"
            }`}
          >
  ...
  return (
    <div className="bg-neutral-50 dark:bg-neutral-900/50 border-b border-neutral-100 dark:border-neutral-700">
      {/* Wide screens: grid-align with table columns */}
      <div className="hidden md:grid" style={{ gridTemplateColumns: GRID_COLS }}>
        <div className="col-span-4" />
        <div className="col-span-2 py-2 pr-6 min-w-0">{editorCard}</div>
      </div>
      {/* Narrow screens: full-width with padding */}
      <div className="md:hidden px-4 py-2">{editorCard}</div>
    </div>
  );
}
